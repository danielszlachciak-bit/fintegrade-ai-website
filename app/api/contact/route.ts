...
const contactSchema = z.object({
  ...
  topic: z.enum(["micro_product", "controlling_ai", "partnership", "other"]),
...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.success) {
      console.warn("contact_validation_failed", JSON.stringify(parsed.error.flatten()));
    ...
    const { data: insertedRows, error: insertError } = await supabase
      ...
      .insert({
        ...
        privacy_acknowledged: parsed.data.privacyAcknowledged,
    ...
    if (!insertedId) {
      console.error("contact_supabase_error", "Insert completed, but Supabase did not return the record ID.");
      return NextResponse.json({ error: "database_error" }, { status: 500 });
    }

    console.info("contact_message_saved", { id: insertedId, topic: parsed.data.topic });

    return NextResponse.json({ ok: true, messageId: insertedId }, { status: 201 });
  } catch (error: unknown) {
    console.error("contact_submit_failed", formatUnknownError(error));
